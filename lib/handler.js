...
module.exports = (db, options, callback) => {
  return req => {
    ...
    if (req.method !== 'GET') {
      const _method = req.body ? req.body._method : null;

      if (req.method === 'PUT' || _method === 'PUT') {
        params.action = 'create';
      }

      if (req.method === 'PATCH' || _method === 'PATCH') {
        params.action = 'update';
      }

      if (req.method === 'DELETE' || _method === 'DELETE') {
    ...
    return _resourceHook({
      ...
      resource: Model ? _resBuilder(db, req, Model, params, options, callback) : undefined,
      params: req.query,
