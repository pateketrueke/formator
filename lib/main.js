...
const { _getCachedFiles } = require('./util');
...
module.exports = (db, options, callback) => {
  ...
  return (req, res, next) => {
    ...
    const _cached = _getCachedFiles();

    if (_cached[req.path]) {
      const { headers, filepath } = _cached[req.path];
      ...
      .then(data => {
        ...
        if (!json) {
          ...
          if (process.env.NODE_ENV !== 'production') {
            scripts = '<script src="vendor.js"></script><script src="main-iife.js"></script>';
          }
          ...
          res.write(`<!DOCTYPE html><html><head>
            ...
            <body><script type="application/json" rel="resource">${
